<template>
    ...
    <!-- <masonry-wall :items="todaysEvents" :ssr-columns="1" :column-width="300" :gap="15">
        <template #default="{ item, index }" >
            <div style="height: min-content; max-width: 400px;"  class="event">

                <div class="event-date">
                        <p>{{ new Date(new Date(this.$store.state.selectedDate).setDate(new Date(this.$store.state.selectedDate).getDate() + 1)).toDateString() }}</p>
                        <p style="font-size: 12px">{{ todaysEvents.indexOf(item)+1 }} of {{ todaysEvents.length }}</p>
                </div>

                <eventPic :event="item"/>

                <p class="event-title" v-html="formatWithKeywords(item.title, item.keywords)"></p>

                <eventMap :event="item"/>

                <eventMedia :event="item"/>
...
export default {
    ...
    components: { eventMap, eventMedia, eventPic, eventSearch, MasonryWall },
